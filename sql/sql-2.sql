...
VALUES
  ...
  -- 데이터 조회건수를 제한
  -- LIMIT 건수
  select * from contact order by email desc limit 20;
  
  -- OFFSET 건수만큼 건너띄기(skip)
  select * from contact order by email desc limit 10 offset 20;
  
  -- 총 데이터 건수 조회
  -- count(인덱스), *로 하면 clustered index의 값 개수만큼을 조회함.
  ...
  -- 조건 검색(contain), LIKE
  -- 전체 데이터를 다 뒤져야 하는 최악의 케이스 ↓ (앞뒤 % 있는 경우.)
  -- 꼭 앞 뒤 %를 써야 한다면 페이지를 정해주는게 낫다.
  select * from contact where name like '%Name1%';
  -- 조건과 일치하는 5개만 조회한 후 종료
  select * from contact where name like '%Name1%' limit 5;
  -- 특정 키위드로 시작하는 검색
  -- '이'로 시작하는 이름은 몇명일까?
  select * from contact where name like 'Name1%';
  ...
  -- PK 기준으로 현재 페이지의 마지막 PK를 저장하고, 마지막 PK 기준으로 LIMIT만 처리함.
