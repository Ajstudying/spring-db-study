...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if(handler instanceof HandlerMethod){
            ...
            //@Auth 어노테이션이 없으면 토큰 관련 인증처리를 별도로 하지 않음.
            if(auth == null){
                return true;
            }
            // 2. 인증 토큰 읽어오기
            ...
            //인증 토큰이 있으면
            // 3. 인증 토큰 및 페이로드(subject/claim) 데이터 객체화 하기
            //메세지 개념에서 서로 주고받는 데이터를 페이로드(payload)
            AuthProfile profile = jwt.validateToken(token.replace("Bearer ", ""));
            if(profile == null){
                //401: Unauthorized(토큰이 잘못 됨)
                // 인증 토큰이 잘못 됨(시그니처, 페이로드, 알고리즘 등...)
                response.setStatus(401);
                return false;
            }
